...
RitmosGUIArcade {
	...
	*new { arg player, numVoices;
	...
	init { arg argPlayer, argNumVoices, argMaxLength;
		player = argPlayer ?? { RitmosPlayerArcade.new };
...
//		voices = { IdentityDictionary.new() }!numVoices;
	...
	makeGUI { arg argBounds;
		...
		voices = Array.fill(numVoices, {|i|
			var res = RitmosVoiceGUIArcade.new(layout, i, player);
			layout.hr(height: 5);
			layout.startRow;
			res;
		});
...
// ... finish		
	...
	updatePreset {
		...
		if(player.countLimitFunc.notNil,{
...
//		var o = 0@0;
		...
		presetView = SNBox.new(pLay, Rect(0, 0, 40, 20) )
			.step_(1)
			.align_(\center)
			.value_(0)
...
//		o = 0@20;
		...
		goalView = SNBox.new(pLay, Rect(0, 0, 40, 20) )
			.step_(1)
			.align_(\center)
			.value_(1)
...
//[lay, name, bounds, envir].postln;
	...
	// make snbox with label and interpolate switch
...
// TODO figure out good fixed sizes, don't use labelRatio
			text: GUI.staticText.new(lay, Rect(0,0,width*labelRatio, height))
				.string_(name)
				.align_(\right),
// NOTE SNBox is not GUI...so not PC compatible
			view: SNBox.new(lay, Rect(0,0,width*numberRatio, height))
				.action_(action)
				.value_(value)
				.clipLo_(clipLo)
				.clipHi_(clipHi)
...
// TODO fix the whole interpolation switch envir thing -- dummy right now
			switch: RitmosGUI.makeInterpolateSwitch(lay, name, envir: envir)
		);
...
RitmosVoiceGUIArcade {
	...
	var <>sideLayout, <>paramSelectView, <>synthEditButton;
	
	var <>controlLayout, <>weightView, <>numGenView, <>mutRateView;
...
// TODO inital value should be given by input rhythm array
		...
		rhythmEditCells = Array.fill(numCells, {|i|
			var pos = o.x + (i*boxWidth);
			SNBox.new(rhythmLayout, Rect(pos, o.y, boxWidth, editCellHeight) )
				.canFocus_(true)
				.align_(\left)
				.font_(Font(skin.fontSpecs))
				.value_(0)
	...
	//				var voice = player.voices[name];
	//				paramRhythmLists.at(currentParameter)[i] = view.value;
	//				if((i < voice.rhythm.size),{
	//					voice.params.at(currentParameter).put(i,view.value);
	//					voice.voiceEnvir.put(currentParameter,voice.params[currentParameter]*1000);						if(currentParameter == \amp,{ voice.rhythm.put(i,view.value)});
	//				});
	//				view.background_(currentParamColors[0].blend(currentParamColors[1], view.value) );
				})
		});
...
// TODO need to fix the yOffset and generally the gaps of stuff
	makeControlView {
		layout.startRow;
		...
		RitmosGUI.makeInterpolateSwitch(controlLayout, "weights", Rect(120, 0, 10, 39), envir: player.voices[name].intEnvir);
		
		controlLayout.flow({ arg gaValLayout;
			numGenView = RitmosGUI.makeEasyNumBox(gaValLayout, Rect(0, 0, 120, 18), "numGen",
			...
			mutRateView = RitmosGUI.makeEasyNumBox(gaValLayout, Rect(0, 0, 120, 18), "mutRate",
		...
		controlLayout.flow({ arg claveLayout;
			countView = RitmosGUI.makeEasyNumBox(claveLayout, Rect(0, 0, 120, 18), "count",
				{|me| "implement me!".postln },
				0, 0, inf, 1
				, envir: player.voices[name].intEnvir
			);
			
			levelView = EZSlider.new(claveLayout, 220@16, "level", [0,3,\amp,0], 
	//			{|me| player.changeVols(me.value,name+1)},
				{|me| player.voices[name].vol_(me.value)},
				0.5, false, 40, 40
			);

// TODO you'll need to have variable names for these because you'll need to remember sometimes
			RitmosGUI.makeInterpolateSwitch(claveLayout, "level", envir: player.voices[name].intEnvir);
			
			claveLayout.startRow;

			rhythmLengthView = RitmosGUI.makeEasyNumBox(claveLayout, Rect(0, 0, 120, 18), "cycle",
	//			{|me| player.changeCycle(me.value.asInt,name+1) },
...
//							array.copyRange(0,rhythmLength-1) });
	...
	//			{|me| player.changeBeatDivs(me.value.asInt,name+1) },
			...
			GUI.staticText.new(claveLayout, Rect(0, 0, 36, 18) ).string_("inst ").align_(\right);
...
// NOTE this still assumes funky method of making SNBox view (i.e. envir instead of class)
