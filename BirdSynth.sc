BirdSynth {
	classvar <>idCount=0;
	var name,<server,<numChans,<>midiChan,<>ambDims,
	envsPath,phrsPath,<>tempo,start,>show,specs,<>outBus,
	<birdFuncs,<birdGen,<>curBird,<birdArgs,
	...
	<behaviors,defaultArgs,volume=0.35;

	*new { arg name,server,numChans=2,midiChan=1,ambDims,envsPath,phrsPath,tempo=60,
			start=true,show=true,specs,outBus=0;
		^super.newCopyArgs(name,server,numChans,midiChan,ambDims,envsPath,phrsPath,tempo,
	...
	newListener {
		listener= MIDIListener.new(midiChan,unique: true); // each ProxyBird has own listen-history
	...
	startSynth {
		// Ndef.ar(\birdGen,numChans);
		birdGen = NodeProxy.audio(server,numChans);
		// birdGen = Ndef.kr(\birdArgs,10);
		birdArgs = NodeProxy.control(server,10);
		// Ndef(\birdArgs).map(defaultArgs)  // ?? will this work?
		...
		birdGen.source =  birdFuncs[\bird1];
		birdGen.play(outBus);
		birdGen.set(\volEnv,birdArgs);
		this.loadPlayFuncs;
	...
	trg {
		// birdVol.source=lastVol;  // turn up voice volume    -- CREATES A PROBLEM!!
		birdArgs.source = argsLists.collect({|list,i|
			{EnvGen.kr(Env.new(list,Array.fill(199,{199.reciprocal})),timeScale: envDur)}
		});
	...
	makeControls {
		...
		pitchGUI = EZNumber.new(window,100@20,"pitch",\freq,{|gui| birdGen.set(\frq,gui.value.round(0.01))},440,
			labelWidth:40,numberWidth:60);
		ampGUI = GUI.ezNumber.new(window,100@20,"amp",nil,{|gui| birdGen.set(\amp,gui.value)},0.5,
	...
	changeBirdFunc {|name|
		if(birdFuncs[name.asSymbol].notNil,{
			birdGen.source = birdFuncs[name.asSymbol];
	...
	loadBirdSynths {
		 birdFuncs = IdentityDictionary[
			...
			roomSize=0.25,damp=0.25,volume=1;
				var tblBuf,bufPtr,sig,flt,swpLFO,numHrms=7,rng,w, x, y, z, rvb;
				...
				flt = RLPF.ar(sig.poll(5,"sig"),fltFrq+birdFltFrqEnv.kr,rq,fltEnv);
				flt=(flt+(sig*(1-fltEnv)))*(volEnv.poll(2,"volEnv"));
				rvb = FreeVerb.ar(flt,room:roomSize,damp:damp);
				Out.ar(0,Pan2.ar(rvb*birdVol.kr,birdAz.kr))
/*			#w, x, y, z = BFEncode1.ar(rvb*birdVol.kr,birdAz.kr,birdElev.kr,birdRho.kr);
				BFDecode1.ar(w, x, y, z,ambDims[0],ambDims[1],ambDims[2])*/
		}
	/*
			'bird2' -> {
				arg frq=440,amp=0.5,volEnv,fltEnv,
			...
			phrsTask = Task({
				listener.evRhythm(thisPhraseNum).size.do {
					...
					thisOct = Array.series(12,0,1).wchoose(listener.octaves(thisPhraseNum));
						birdGen.set(\frq,thisPitch=((thisNote+(thisOct*12)).midicps));
							{ pitchGUI.value_(thisPitch)}.defer;
					birdGen.set(\amp,thisAmp=(listener.vels(thisPhraseNum)[thisNote]));
							{ ampGUI.value_(thisAmp)}.defer;
					SystemClock.sched((listener.durs(thisPhraseNum)[thisNote]),
							{ birdGen.set(\amp,0); nil });
		...
		'playMelody' -> {|thisPhraseNum,thisPhraseDur|
			...
			phrsTask = Task({
				numNotes.postln.do {|i|
					...
					birdGen.set(\frq,thisPitch=(thisNote.midicps));
							{ pitchGUI.value_(thisPitch)}.defer;
					"amp = ".post;
					birdGen.set(\amp,thisAmp=(listener.vels(thisPhraseNum)[inputMelody[i]%12]).postln);
							{ ampGUI.value_(thisAmp)}.defer;
					"duration = ".post;
					SystemClock.sched((listener.durs(thisPhraseNum)[inputMelody[i]%12]).postln,
						{ birdGen.set(\amp,0); nil }); " ".postln;
					thisDur = rhythm.next.max(0.05);
					thisDur.wait;	// wait dur until next note
				}
			}).play
		...
		'playChord' -> {|thisPhraseNum,thisPhraseDur|
			...
			phrsTask = Task({
				numNotes.do {|i|
					...
					birdGen.set(\frq,thisPitch.midicps);
					birdGen.set(\frq2,(frq2.midicps));
					{ pitchGUI.value_(thisPitch)}.defer;
						birdGen.set(\amp,thisAmp = (listener.vels(thisPhraseNum)[inputMelody[i]%12]));
						{ ampGUI.value_(thisAmp)}.defer;
					thisDur = (rhythm.next.max(0.05));
	...
	quit {
		...
		birdVol.source = 0;
		[ birdGen, birdArgs, birdEnv, birdFrqEnv, birdFltFrqEnv, birdAz,
...
/*
(
SynthDef(\ctlTest,{|cbus=0|
	var sig = SinOsc.kr(Array.fill(10,{rrand(0.1,10)}));
	Out.kr(cbus,sig)
}).send(s);

SynthDef(\sines,{|cbus|
	var ctls = In.kr(cbus,10);
	var sigs = SinOsc.ar(\ffrq.asSpec.map(ctls),0,0.5/10);
//	var sigs = SinOsc.ar(Array.fill(10,{rrand(440,660)}),0,0.5/10);
	Out.ar(0,Splay.ar(sigs))
}).send(s);
)

a = Synth(\ctlTest);
b = Synth.after(a,\sines,[\cbus,0])
b.free

SynthDef('bird1',{| frq=440,amp=0.5,thisDur=1,thisPitch=440,cbus=0|
	var volEnv=0,fltEnv=1,frqDev=2,fltFrq=3,frqModFrq=4,frqModDpth=5,
	rq=6,timbre=7,ampModFrq=8,ampModDpth=9,
	roomSize=0.25,damp=0.25,volume=1;
	var tblBuf,bufPtr,sig,flt,swpLFO,numHrms=7,rng,w, x, y, z, rvb;
	tblBuf = VOscBufs.buffers[0].bufnum;
	bufPtr = tblBuf+(Lag.kr(In.kr(timbre)*numHrms,0.05)).clip(tblBuf,(tblBuf+numHrms)-0.01);
	rng = In.kr(frqModDpth).min(1.0)*bufPtr;
	swpLFO = LFTri.kr(In.kr(frqModFrq),0,rng*0.5,(rng*0.5).neg);
	sig = VOsc.ar((bufPtr+swpLFO),frq+(frq*In.kr(frqDev))+
		EnvGen.kr(Env.perc(0.025,thisDur,thisPitch*2,-12),levelBias: amp)) ;
	sig = sig*(LFPar.ar(In.kr(ampModFrq),0,In.kr(ampModDpth)*0.5,0.5));
	flt = RLPF.ar(sig,In.kr(fltFrq)+birdFltFrqEnv.kr,rq,fltEnv);
	flt=(flt+(sig*(1-In.kr(fltEnv))))*(In.kr(volEnv));
// replace NodeProxies.kr below with their functions
	rvb = FreeVerb.ar(flt,room:roomSize,damp:damp);
	#w, x, y, z = BFEncode1.ar(rvb*birdVol.kr,birdAz.kr,birdElev.kr,birdRho.kr);
	BFDecode1.ar(w, x, y, z,ambDims[0],ambDims[1],ambDims[2])
}).send(s);
