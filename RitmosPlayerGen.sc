...
RitmosPlayerGen {
	...
	var <>currentGoal,<>currentVoiceView=0;
	var <>intEnvir;
 ...
 //	you can comment out or delete this...copied from your startupfile
	...
	init { 
		...
		voices = Array.fill(numVoices,{|i| RitmosVoiceGen.new(this,i) });
...
// TODO nil check?  delete would mess this up...
	...
	//	BUG IS IN loadPreset AFTER SAVE....
...
RitmosVoiceGen {
	...
	var <>player, <voiceNum, <rhythm, <beatDiv, <synth, <>inChan, <>pattern, 
	<>weights,<>numGen=4,<>mutRate=0.25,<>gate=0.1,<>vol=1, <>clock,
		<>count=0, <stream, <>voiceEnvir, <params, <>gui, <guiUpdate, <>ampGate=0.05,
		<downbeat, <rhythmTarg, <>inputBeats, 
		<rhythmCritter, <gA, <breeder, <>breederRate=20,
		<inputAnalyzer, <>outputXfrmFunc;
...
// 6/17 ls
	*new { arg player, voiceNum, rhythm, beatDiv, synth, inChan=1;
	...
	init { 
		...
		weights = [0.5,0.0,0.5];
		// start recording and analyzing MIDI input
		inputAnalyzer = MIDIPhraseAnalyzer.new(inChan);
		if(inputAnalyzer.isRunning.not,{ inputAnalyzer.start });
		// start playing the clave-rhythm
		rhythmCritter = rhythm.copy;
		outputXfrmFunc = 'echo';
// TODO maybe this should get parent envir from player just like specs?
		intEnvir = RitmosVoiceGen.defaultIntEnvir;
		seqEnvir = RitmosVoiceGen.defaultSeqEnvir;
...
// NOTE default key does nothing yet...but might be useful eventually?
	...
	*outputXfrmFuncs {
		^(echo: {|rhyth| rhyth }, opposite: {|rhyth| rhyth.collect {|bt| 1.0-bt }} )
	}
...
// TODO change to default value
	...
	asPreset {
		^(
			...
			vol: vol.copy,
			weights: weights.deepCopy,
			numGen: numGen.copy,
			mutRate: mutRate.copy,
			gate: gate.copy,
			outputXfrmFunc: outputXfrmFunc.copy,
	...
	loadPreset {|preset|
		...
		this.weights_(preset.weights);
		this.numGen_(preset.numGen);
		this.mutRate_(preset.mutRate);
		this.gate_(preset.gate);
		this.outputXfrmFunc_(preset.outputXfrmFunc);
		
		{ // set gui display to values from preset
			this.gui.instSelectView.value_(
				this.gui.instrumentList.indexOf(this.voiceEnvir.at(\instrument)));
			...
			this.gui.weightView.value_(this.weights);
			this.gui.numGenView.view.value_(this.numGen);
			this.gui.mutRateView.view.value_(this.mutRate);
			this.gui.gateView.view.value_(this.gate);
		}.defer;
	}
	...
	breedRhythm {
		...
		rhythmCritter = rhythmCritter.collect {|x| if((x>gate),{x},{0})}; // gate out low values
	...
	getInputBeats {
		...
		if(downbeat.notNil,{
			...
			dbTime = downbeat - inputAnalyzer.startTime + Server.default.latency;
			while({inputAnalyzer.onTime(cycCtr) > (dbTime-(cycTime/rhythm.size*0.5))},{ 
				cycCtr = (cycCtr+1) });
//			if(voiceNum == player.currentVoiceView,{ "dbTime = ".post; dbTime.postln; });
			lastCycInput = Array.fill(cycCtr,{|i| 
				[inputAnalyzer.onTime(i),inputAnalyzer.velNum(i)]});
			lastCycInput.do {|vals| var ictus;
				ictus = (vals[0] - dbTime) / (cycTime / rhythm.size);
//				if(voiceNum == player.currentVoiceView,{ ictus.postln; });
	...
	newRhythmTarg {
		...
		this.getInputBeats;
		if(voiceNum == player.currentVoiceView,{ 
			inputBeats.round(0.001); });
	...
	rhythmToDurs {|rhyt|
		...
		durs = rhyt.inject([],{|array,val,i| if(((val>gate) || (i == 0)),{array ++ (i+1) },{array }) });
		^durs = durs.collect {|item,i| 
			if((i==0) && (durs.size > 1),{ 
				durs[1]-item 
			},{ 
				if( (i<(durs.size-1)),{ durs[i+1]-item
		 		},{ (rhyt.size+1)-item 
		 		})
		 	})
		 };
	...
	makeEventCycle {
		^Pn( Plazy {
			var noteDurs, indexes, event, outputRhythm;
			this.newRhythmTarg; 
			// store downbeat
			downbeat = thisThread.seconds;
			if(voiceNum == player.currentVoiceView,{ 
				rhythmTarg.round(0.001);   
			});
			outputRhythm = RitmosVoiceGen.outputXfrmFuncs[outputXfrmFunc].value(rhythmCritter);
			noteDurs = this.rhythmToDurs(outputRhythm);
			if(voiceNum == player.currentVoiceView,{ 
				 outputRhythm.round(0.001); 
			});
			indexes = [0]++(noteDurs.integrate.drop(-1));
			event = voiceEnvir.copy;
			event.keysValuesChange {|key, value, i|
				Pseq(value.asArray.wrapAt(indexes),1);
			};
			event.put(\instrument,synth);
			event.put(\amp, Pseq(outputRhythm.wrapAt(indexes)
					*player.ampMax*vol*player.masterVol, 1));
			event.put(\dur, Pseq(noteDurs, 1));
...
//							lastColor = lastCell.background;
		...
		breeder = Routine { 
			loop {  this.breedRhythm; 
				breederRate.reciprocal.wait }
		}.play(AppClock);
...
//			params = params.collect({|array| array.copyRange(0,newCycle-1)});
	...
	stop { 
		stream.stop; breeder.stop; count = 0; downbeat = nil;
		inputAnalyzer.stop;
