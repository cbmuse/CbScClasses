...
KnowNoSynthPlayer {
	var network,<vcNum,<view,<analyzers,synthDefs,<synth,<curSynth,
	synthModCtls,<pguis,synthOnOff,<synCtl1,<synCtl2,<synCtl3,<curCtls,
	mod1Ctl,mod2Ctl,mod3Ctl,manCtl,volCtl,<>synCtlFunc;
	...
	*new {|network,vcNum,view| ^super.newCopyArgs(network,vcNum,view).init }
	...
	makeGui {
		...
		view.flow({|p|
			...
			pguis = [Button.new(p,80@20)
				...
				Slider.new(p, 355@20).action_({|sl| synth.notNil.if({synth.set(\manCtl,sl.value)})}),
				p.startRow;
				volCtl = StaticText.new(p,80@20).string_("vol").align_(\right);
				Slider.new(p, 355@20).action_({|sl| synth.notNil.if({synth.set(\vol,sl.value)})});
		...
		synthOnOff.action_({|cv| var name= pguis[1].items[pguis[1].value];
			if(cv.value==1,{ synth=Synth(name,[\vc,vcNum]) },{ synth.release; synth=nil })});
		curSynth.action = {|m|
			...
			if(synth.notNil,{{ synthOnOff.value_(0); // stop running synth
				pguis[6].value_(0.2) // reset 'vol' slider to 0.2
			}.defer });
		};
		synCtl1.action_({|ctl| synth.notNil.if({synth.set(curCtls[0],ctl.value)})});
		synCtl2.action_({|ctl| synth.notNil.if({synth.set(curCtls[1],ctl.value)})});
		synCtl3.action_({|ctl| synth.notNil.if({synth.set(curCtls[2],ctl.value)})});
	...
	loadSynthDefs {
		synthDefs = [
			SynthDef(\fmFB,{|freq=1000,mfrq=0.1,idx=2,amp=0.1,vc=0,gate=1,manCtl=0.1,vol=0.2|
			var sig,env;
			...
			sig = SinOsc.ar(
				SinOsc.ar(
					...
					freq),0,
				amp*env);
			Out.ar(analyzers.fbBus.index+vc,sig);
			Out.ar([0,1],sig*vol)
			}),
			SynthDef(\fmDrumFB,{ arg freq=440,rtio=4,idx=2,modFrq=0.1,mod=0,trgf=4,
				phs=0,att=0.01,rls=0.25,amp=0.1,vc=0,gate=1,vol=0.2,manCtl=0.1;
				var modSig,env,sig;
				freq=\freq.asSpec.map(manCtl);
				env = EnvGen.kr(Env.asr(attackTime: 0.001, releaseTime: 0.025),
					gate, doneAction: 2);
			...
			SynthDef(\pluckFB,{ arg freq=400,manCtl=0.1,brit=2000,plSpd=4,plDcy=1,clip=0.2,rq=0.5,
				mfrq=0.2,mod=1,att=0.01,rls=0.5,amp=0.1,vc=0,vol=0.2, gate=1;
			...
			SynthDef(\motoRevFB,{|freq=0.5,manCtl=0.1,lfrq=4,fmul=20,wdth=0.2,ffrq=2000,
				clip=1,amp=0.1,vc=0,gate=1,vol=0.2|
				var env,sig;
				freq=\freq.asSpec.map(manCtl);
			...
			SynthDef(\steam, {arg manCtl=0.1,prm1=0.5, prm2=0.5, amp=0.2,
				...
				Out.ar(fbBus,Mix.new(sig));
				Out.ar([0, 1], sig*vol);
			}),

			SynthDef(\gsLqdSld, {arg manCtl=0.1, prm1=0.5, prm2=0.5, amp=0.2, vc=0,  gate=1, vol=0.2;
				var sig, env;
				...
				clock = {Dust.kr(clockRate*manCtl, 0.1)}.dup;
				...
				Out.ar(analyzers.fbBus.index+vc,sig);
				Out.ar(Pan2.ar(sig,0,vol));
			}),

			SynthDef(\prickles, {arg prm1=0.5, prm2=0.5, amp=0.2, vc=0,  gate=1, manCtl=0.1,vol=0.2;
				...
				sig = sig*env*amp;
				Out.ar(analyzers.fbBus.index+vc,Mix.new(sig));
				Out.ar([0, 1], sig*vol);
			}),

			SynthDef(\noStation, {arg prm1=0.5, prm2=0.5, amp=0.2, vc=0,  gate=1, manCtl=1.0, vol=0.2;
				var sig, env, freq;
		...
		].collect {|def| def.send(analyzers.s) };
