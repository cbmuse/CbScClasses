MIDIListener {
	...
	var <>midiChan, <>bounds, <>unique, <>history, <>phrases,
	<>break=1.0, <>startTime, <>pauseTime, <>pauseSum=0, <>pauseFlg=false,
	...
	*new { arg midiChan=1, bounds=[21,108], unique=false;
		var listenList, match;
		if(unique,{  // if unique, multiple listeners can exist with same chan and bounds
			^super.newCopyArgs(midiChan,bounds,unique).initHistory
			},{ // if not unique, and this is first on this channel, make a new one
				if( MIDIListener.listeners[midiChan-1].isEmpty,{
					^super.newCopyArgs(midiChan,bounds,unique).initHistory
					},{ // if not unique, and midiChan and bounds are same, use previous
						listenList = MIDIListener.listeners[midiChan-1];
						match = listenList.detectIndex {|list| (list.bounds == bounds) };
						if(match.notNil,{ ^MIDIListener.listeners[midiChan-1][match] },{
							^super.newCopyArgs(midiChan,bounds,unique).initHistory
						})
				})
		})
	...
	initHistory {
		// add non-unique listeners to list
		if(unique.not,{MIDIListener.listeners
			.put(midiChan-1,MIDIListener.listeners[midiChan-1].add(this))});
	...
	stop {
		...
		ccResponder.remove;
		which = MIDIListener.listeners[midiChan-1].detectIndex {|list| list.bounds == bounds };
		if( which.notNil,{
			MIDIListener.listeners[midiChan-1].removeAt(which);
	...
	reset {
		...
		phrases = List.new(100);
		startTime = thisThread.seconds;
		pauseTime=startTime; pauseSum=0; pauseFlg = false;
		activityLevels = [0,0,0,0];
	}
	...
	storeNoteOn { arg chan,num, vel;
		...
		if((isRunning && pauseFlg.not),{
			if( showInput,{ "noteOn = ".post; num.post; " ".post; vel.postln; });
...
//			"new note at ".post; now.postln;
	...
	storeNoteOff { arg chan,num, vel;
		...
		sameNote = history.detect({ arg list, i; thisOne = i;
			(list.at(2) == num) // noteNum matches
			&& (list[8] != "nil") });  // and note hasn't already been turned off
		// "sameNote= ".post; sameNote.postln;
		if( sameNote.notNil,{
 ...
 [startTime, duration, pitchWeights, octaveWeights, avgVels/note, avgDur/note, avgVel/phrase, avgDur/phrase, numNotes,avgEvDens,numEvents,rhythm,melody,chordnotes, pauseSum ]
	*/
	analyzePhrases {
		...
		if(this.history.isEmpty.not,{
			...
			//	"lastPhrase = ".post; lastPhrase.postln;
...
/* [startTime, duration, numNotes, pitchWeights, octWeights, avgVels/note, avgDur/note, avgVel/phrase, avgDur/phrase, avgEvDens,numEvents,rhythm,melody, chdNotes, pauseSum ] */
						analysis = [
							...
							numEvents, rhythm, melody, chdNotes, pauseSum ];
		...
		monitorRout = Routine({
			loop({
				...
				if(phrases.size > 0,{
					...
					recentOnDurs = phrases.copyFromStart(secStart).sum({|phr| phr[1] });
					activityLevels.put(2,recentOnDurs/
						(pieceTime - (this.phraseStartTime(secStart)-this.phrasePauseSum(secStart))));
					...
					lastRestDur = thisThread.seconds - startTime - lastPhrEndTime;
...
/*
m = MIDIListener.new(1)
~listener.phrases
~listener.phraseNum(0)
~listener.phraseStartTime
~listener.history.do {|n| n.postln }
~listener.view
~listener.showAnal
*/