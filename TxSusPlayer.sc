	...
	*new {|argName,argChan=0,argPrstPath,argTuning|
	...
	init {|argName,argChan,argPrstPath,argTuning|
		...
		fmArgs = (\waves: CV.new.sp([0,0,0],0,7,1,\lin),
			...
			\rtios: CV.new.sp([1,2,3,4],0.125,16,0.25,\lin),
			\fxFreqs: CV.new(\widefreq,[0.1,0.1,0.1,0.1]),
			\times_1: CV.new(nil,[0.01,0.1,0.5,0.1]),\points_1:CV.new(nil,[1,0.5,0.05]),
			\times_2: CV.new(nil,[0.01,0.1,0.5,0.1]),\points_2:CV.new(nil,[1,0.5,0.05]),
			\times_3: CV.new(nil,[0.01,0.1,0.5,0.1]),\points_3:CV.new(nil,[1,0.5,0.05]),
			\times_4: CV.new(nil,[0.01,0.1,0.5,0.1]),\points_4:CV.new(nil,[1,0.5,0.05]),
			\rlsNode: CV.new.sp(2,0,3,1,\lin), \loopNode: CV.new.sp(1,0,2,1,\lin),
			\lfoWvs: CV.new.sp([0.1,0,0,0],0,1,0,\amp),
			\lfoSpd: CV.new.sp(0.5,0,20,0,\lin),\pmod: CV.new(nil,0),\amods: CV.new(nil,[0,0,0,0]),
			\alg: CV.new.sp(1,1,8,1,\lin),\freq: CV.new(\freq,440),\sustain: CV.new.sp(1,0,30,0,\lin),
			\pan: CV.new(\bipolar,0),\fb: CV.new(nil,0),\out: CV.new.sp(0,0,15,1,\lin),
			\amp: CV.new(nil,1),\ffrq: CV.new.sp(20000,100,20000,nil,20000),
			\effOut: CV.new.sp(16,16,24,nil,1), \effAmp: CV.new(nil,0),
	...
	playGuiSynth {|freq,amp|
		if(freq.isNil,{freq=this.get(\freq)});if(amp.isNil,{amp=this.get(\amp)});
		synth=Synth("txSusAlg"++(this.get(\alg).asString),[
			\freq,freq,\amp,amp,
			\sustain,this.get(\sustain),\pan,this.get(\pan),\fb,this.get(\fb),
			...
			\wv1,TxSusSynthDefs.buffers[this.get(\waves)[0]],
			\wv2,TxSusSynthDefs.buffers[this.get(\waves)[1]],
			\wv3,TxSusSynthDefs.buffers[this.get(\waves)[2]],
			\amp1,this.get(\amps)[0],\amp2,this.get(\amps)[1],
			\amp3,this.get(\amps)[2],\amp4,this.get(\amps)[3],
			\t1_1,this.get(\times_1)[0], \t2_1,this.get(\times_1)[1],
			\t3_1,this.get(\times_1)[2],\rls_1,this.get(\times_1)[3],
			\p1_1,this.get(\points_1)[0],\p2_1,this.get(\points_1)[1],\p3_1,this.get(\points_1)[2],
			\t1_2,this.get(\times_2)[0],\t2_2,this.get(\times_2)[1],
			\t3_2,this.get(\times_2)[2],\rls_2,this.get(\times_2)[3],
			\p1_2,this.get(\points_2)[0],\p2_2,this.get(\points_2)[1],\p3_2,this.get(\points_2)[2],
			\t1_3,this.get(\times_3)[0],\t2_3,this.get(\times_3)[1],
			\t3_3,this.get(\times_3)[2],\rls_3,this.get(\times_3)[3],
			\p1_3,this.get(\points_3)[0],\p2_3,this.get(\points_3)[1],\p3_3,this.get(\points_3)[2],
			\t1_4,this.get(\times_4)[0],\t2_4,this.get(\times_4)[1],
			\t3_4,this.get(\times_4)[2],\rls_4,this.get(\times_4)[3],
			\p1_4,this.get(\points_4)[0],\p2_4,this.get(\points_4)[1],\p3_4,this.get(\points_4)[2],
			\rlsNode,this.get(\rlsNode),\loopNode,this.get(\loopNode),
			...
			\out,this.get(\out),
			\ffrq,this.get(\ffrq),\effOut,this.get(\effOut),\effAmp,this.get(\effAmp),
			\vol,this.get(\vol)
			], target: group
		);
	...
	loadPreset {|name|
		...
		{ selectPresetView.value_(selectPresetView.items.indexOf(name));
			interpSliderView.value_(0); presetNameView.value_(presetName) }.defer(1)
	}

	loadInterpPreset {|name|
		{ interpPresetNameView.valueAction_(interpPresetNameView.items.indexOf(name)) }.defer
	}
	...
	loadStringToPreset	{|loadPre,curPre|
		loadPre.keysValuesDo {|k,v|
			if(curPre[k].notNil,{ curPre[k].value_(v.value)})
		};
	}
	...
	makeLayout {
		...
		layout.decorator.gap = TxSusSynthDefs.skin.gap;
		layout.decorator.margin = TxSusSynthDefs.skin.margin;
	}
	...
	makePresetControls {
		...
		.font_(Font(TxSusSynthDefs.skin.fontSpecs[0],TxSusSynthDefs.skin.fontSpecs[1]))
		.action_({|view|
		...
		.action_({|b| if( b.value==1,{
			this.playGuiSynth },{if(synth.notNil,{ synth.release; synth=nil})})});
	}
	...
	gui { var chanView;
		...
		valGUIs= fmValues.collect({|name,i|
			...
			.action_({|nv| if(synth.notNil,{ if(synth.isRunning,{ this.setSynthValue(name)}) }) });
			fmArgs[name];
		});
...
p.keys
u.susNotes
u.synths[0][48][0].release
u.susNotes
.do {|n| n.postln }
u.susNotes
u.synths[1].do {|n| n.postln }
.select {|n| n.notEmpty }
				z.fmArgs[\waves].value
z.setSynthArray(\times_2)
z.fmArrayArgNames[\times_2][1]
z.fmArgs[\times_2].value
z.setSynthValue(\freq)
z.fmArgs[\fxFreqs].value
Synth("txSusAlg2" : 1002).release
Server
s.default_group
x=[3.midiratio.reciprocal, 3.midiratio, \exponential].asSpec;
\exponential].asSpec;
u.pbends.put(0,1)
*/