...
RitmosPlayerArcade {
 ...
 //	you can comment out or delete this...copied from your startupfile
	...
	init { 
		...
		voices = Array.fill(numVoices,{|i| RitmosVoiceArcade.new(this,i) });
...
RitmosVoiceArcade {
	...
	var <>player, <voiceNum, <rhythm, <>pattern, <beatDiv, <synth, <>vol=1, <>clock,
		<>count=0, <stream, <>voiceEnvir, <params, <>gui, <guiUpdate, ampMax;

// TODO is this the best way to store info?  should it all be in one envir?
	var <>intEnvir, <>seqEnvir;
	
// 6/17 ls
	*new { arg player, voiceNum, rhythm, pattern, beatDiv, synth;
		^super.newCopyArgs(player, voiceNum, rhythm,pattern,beatDiv,synth).init
	}

	init { 
		ampMax = player.ampMax;
...
// TODO maybe this should get parent envir from player just like specs?
		intEnvir = RitmosVoiceArcade.defaultIntEnvir;
		seqEnvir = RitmosVoiceArcade.defaultSeqEnvir;
...
// TODO change to default value
	...
	asPreset {
		^(
			...
			// don't need clock info because derive from rhythm and beatDiv
			// don't think we need count
			// don't need ampMax, get from parent
		)
	}
	...
	makeEventCycle {
		^Pn( Plazy {
			var noteDur = this.rhythmsToDur;
			var indexes = [0]++(noteDur.integrate.drop(-1));
			var event = voiceEnvir.copy;
			event.keysValuesChange {|key, value, i|
				Pseq(value.asArray.wrapAt(indexes),1);
			};
			event.put(\instrument,synth);
			event.put(\amp, Pseq(rhythm.wrapAt(indexes)*ampMax*vol*player.masterVol, 1));
	...
	rhythmsToDur {|voice|
		var dur;
		if(rhythm[0] == 0,{ rhythm.put(0,0.0001) });  // first beat must not be empty
		dur = rhythm.inject([],{|array,val,i| if(val>0,{array ++ (i+1) },{array }) });
		^dur = dur.collect({|item,i| 
			if((i==0) && (dur.size > 1),{ dur[1]-item 
			},{ 
